...
//Pin for ESP32
  ...
  #define TFT_DC         32 //AO connect to pin 32  try pin 25
...
//But the screen is brightest when 3.3 volts for VCC and 5volts for LED.
  ...
  int w = tft.width();  //defining the width
  int h = tft.height(); //defining the height
void setup() {
    Serial.begin(115200);
    // Use this initializer if using a 1.8" TFT screen:
    tft.initR(INITR_BLACKTAB);      // Initializing the TFT with BlackTAB
...
void loop() {
  ...
  // a single pixel
  tft.fillScreen(ST77XX_BLACK); 
  tft.drawPixel(w/2-5, h/2-5, ST77XX_RED);  
  tft.drawPixel(w/2, h/2, ST77XX_GREEN);
  tft.drawPixel(w/2+5, h/2+5, ST77XX_BLUE);
  delay(1000);
  tft.fillRect(w/2-5,h/2-5,11,11, ST77XX_BLACK);  //fills the pixels with black color so it can makes a pulse effect
  delay(500);
  tft.drawPixel(w/2-5, h/2-5, ST77XX_RED);  
  tft.drawPixel(w/2, h/2, ST77XX_GREEN);
  tft.drawPixel(w/2+5, h/2+5, ST77XX_BLUE);
...
//uncomment the for loop if you want to see the brightness increase from 150 to 255 max(3.3 volts)

/*
for (int i = 150; i <256; i++) { // i = 255 = approx 3.3 volts, i = 0 = approx 0.0 volts

  dacWrite(25, i);      //writing the value to that pin
  tft.setCursor(20, 80); //setting the cursor in that position
  tft.print(i);          //printing the analog value as i
  delay(100);
  tft.fillRect(20,80,40,15, ST77XX_RED);  //filling the print with red color
}
*/
...
// no difference if Vcc is 5v or 3.3v
  ...
  //a filledrounded rectangle
  tft.fillRoundRect(10, 20, w-10, h-20, 5, color);
  delay(1000);
  tft.fillScreen(ST77XX_BLACK);
  delay(500);
  ...
  //a filled triangle x1,y1, x2,y2, x3,y3, color
  color +=1000;
  tft.fillTriangle(10, 20, 0, 50, w, h, color);
  delay(1000);
  tft.fillScreen(ST77XX_BLACK);
  delay(500);

  //a Char x,y,unsigned char(ascii value of that char),color,background color,size
  color +=1000;
  tft.drawChar(w/2, h/2, 88, color, ST7735_CYAN, 2);  //
  delay(1000);
  tft.fillScreen(ST77XX_BLACK);
  delay(500);
  ...
  tft.invertDisplay(true);    //inverts the color
  delay(1500);
